...
class Page(object):
	...
	def save(self, text = u'', summary = u'', minor = False):
		...
		data = {}
		if minor: data['epminor'] = '1'
		if not minor: data['epnotminor'] = '1'
		if self.edit_time: data['epbasetimestamp'] = time.strftime('%Y%m%d%H%M%S', self.edit_time)
		
		result = self.site.api('edit', eptitle = self.name, eptext = text, 
			token = self.get_token('edit'), **data)
	...
	def delete(self, reason = '', watch = False, unwatch = False, oldimage = False):
		...
		result = self.site.api('delete', title = self.name, 
				token = self.get_token('delete'), **data)
